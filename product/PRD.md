...
30. Synthetic flat, hierarchical, HeavyBid-style, reference, operational-reference, review-UX, onboarding, package-integrity/semantic-preview, archived-review continuation, accessibility, bulk-review planning/application/UI, view-context, Review Delta export/verification, Review Timeline, Timeline-export core/download, and control-gate fixtures with regression coverage.
...
35. Explicit deterministic public-runtime composition: one startup composer invokes the idempotent accessibility, bulk-review, review-view-context, operational-reference, archived-review, Review Delta, Review Delta verification, Review Timeline, and Timeline-export download feature installers. Content modules such as onboarding are side-effect free, and review-only renderer augmentation uses stable internal page identity rather than user-visible display-title strings.
...
37. Deterministic Review Timeline evidence-export core: `civil-estimate-review-timeline-export` v1 is implemented as an in-memory library builder plus independent verifier. Creation uses only 2–10 freshly verified Delta v1 bundles, the same fail-closed linear SHA-256 chain, and full verified rows including `UNCHANGED`; it emits the exact nine-member deterministic JSON/CSV/README/integrity ZIP contract and excludes original Delta/review-package/source bytes, persistence, timestamps, generated narratives, scores, Operational Crew/Production evidence, and HeavyBid behavior.
38. Request-local Review Timeline export distribution: `/export-review-timeline` requires the user to freshly select 2–10 Delta v1 ZIPs on that route, reuses the bounded Timeline multipart envelope, invokes the strict export core, and independently verifies the generated Timeline ZIP again before sending response bytes. It carries no Timeline preview/verification/lineage state, creates no review session or persistence, and does not accept Timeline-export ZIPs as browser input.
...
- Review Delta comparison/export/verification, neutral multi-snapshot Review Timeline, deterministic Timeline-export core, and request-local browser download are built; any future Timeline-export browser **verification input** must remain a separate read-only verifier and must not become continuation/restoration/persistence authority;
...
- if Crew Code / Production Rate evidence is ever added to portable review packages, define a new explicit package/version compatibility contract and update semantic verification, archived continuation, Review Delta, and Review Timeline together rather than overloading package-v1 reference rows;

...
12. The review package includes `integrity.json`, which records byte size and SHA-256 for every other member. The in-memory verifier can detect changed, missing, duplicate, unexpected, unsafe, or unsupported package contents. A valid integrity check does not restore the session or establish estimator approval, reference authority, bid readiness, or HeavyBid import validity.
13. Close the application. Current estimate, review, findings-view, reference-view, and reference evidence state exists only in temporary process memory and expires with the local session/process.

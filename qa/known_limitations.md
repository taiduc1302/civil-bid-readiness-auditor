...
- A deterministic evidence-only Review Timeline export v1 contract is documented, but no Timeline export builder, verifier, route, download control, persistence, or accepted Timeline-export input exists. The current bounded browser preview is not a portable full-evidence export.

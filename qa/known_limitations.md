...
- Operational Crew Code / Production Rate comparison is temporary-session evidence only and is not part of review-package v1, archived continuation, Review Delta, or Review Timeline; historical cost/rate fields remain ignored by that comparator.
...
- Review Timeline can structurally order 2–10 independently verified Review Delta evidence ZIPs only when their exact review-package SHA-256 lineage forms one connected acyclic linear chain. It is evidence chronology only: it does not infer calendar dates, elapsed time, source currency, commercial revision identity, estimate quality, improvement/regression, approval, bid readiness, or HeavyBid import validity. Each Delta ZIP remains limited to 50 MB and the dedicated Timeline multipart request is bounded to 502 MB including form overhead.
- Review Timeline is request-local and read-only. It creates no persistent timeline database or review session and does not restore source files, rerun audit/reference logic, or reconstruct session-only Operational Crew/Production evidence.
- The deterministic evidence-only Review Timeline export v1 now has an in-memory core builder and independent verifier. It has **no browser route, download control, persistence, or accepted Timeline-export upload/input surface**, so the existing bounded Timeline browser preview remains separate from portable full-evidence export creation. Verification proves only the exported ZIP's internal structure, hashes, deterministic regeneration, and semantic consistency; it does not re-authenticate absent original Delta/review-package/source bytes.

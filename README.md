...
7. Record temporary local finding dispositions (`Open`, `Reviewed`, `Accepted`, `Needs correction`, or `Suppressed`). Suppression requires a reason and never changes the source estimate or deterministic finding. For a bulk human action, check only the individual rows you intend to change, choose the target status/reason, acknowledge ownership, preview the exact selected findings without mutation, then separately confirm the one-time plan. Filters, grouping, and hidden rows never become implicit bulk scope; stale or replayed plans fail closed before one atomic session assignment.

...
15. **Review Delta** compares an Earlier and Later verified review-package snapshot in memory. It distinguishes deterministic finding-evidence changes from human review-status/reason changes, separately reports governed reference-check drift and reference metadata drift, and counts unchanged rows without showing them by default. Duplicate comparison anchors fail closed. The comparison creates no session, reruns no audit/reference logic, and never interprets change as improvement, correctness, approval, bid readiness, or HeavyBid import validity.
...
See `product/operational_reference_ui_contract.md` for the Crew Code / Production Rate browser-evidence boundary.
